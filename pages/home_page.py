...
class Homepage_guvi():
    ...
    def menu_items_display_enable(self):
        try:
            ...
            element_to_hover = self.driver.find_element(*self.courses_locator)
            ...
            element_to_hover =self.driver.find_element(*self.practice_locator)
            self.actions.move_to_element(element_to_hover).perform()
            self.wait.until(ec.visibility_of_element_located(self.liveclasses_locator))
            element_to_hover= self.driver.find_element(*self.liveclasses_locator)
            self.actions.move_to_element(element_to_hover).perform()
            return True
        except Exception as e:
            print(f"Exception: {e}")
