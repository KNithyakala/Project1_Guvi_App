...
class Test_Guvi_app():
    ...
    def test_validate_url(self,driver,get_url):
        try:
            ...
            driver.get(get_url)
            assert driver.current_url == get_url
            logging.info("The application is loaded successfully.")
            driver.save_screenshot("my_screenshots\Pass_testcases\TC01_Pass.png")
        except Exception as e:
            logging.info(f"Expected {get_url}, but got {driver.current_url}")
            driver.save_screenshot("my_screenshots\Fail_testcases\TC01_Fail.png")

    # Test Case 2 - Validating title
    def test_validate_title(self,driver,get_title):

        try:
            assert driver.title == get_title
            logging.info(f"The title is displayed as {driver.title}.")
            driver.save_screenshot("my_screenshots\Pass_testcases\TC02_Pass.png")

        except Exception as e:
            logging.info(f"Expected Title:{get_title}, but title of page is displayed as{driver.title}")
